...
| 时代 | 开局 | 版图 |
|---|---|---|
| 二战前夕 · 梦魇 | 1938 年 1 月 | 奥地利尚未并入德国，英国仍持巴勒斯坦与印度，日本据有朝鲜。欧洲是和平的，中国已经打了半年，**开局即在战争中** |
...
**国旗和简介也都随时代变**。1938 年的中国挂的是中华民国国旗、俄罗斯是苏联国旗、美国是 48 星旗、印度是英属印度旗、伊朗是狮日旗、巴西是 1889 年版；德国是德意志国旗（见下）。这些是**从维基共享资源直接下载的原图**（`scripts/fetch-flags.py`，无人手绘），脚本把它们套进与现代国旗相同的 36×36 画布，所以游戏里所有国旗都是同一种东西——地图图标、面板、标题界面共用一套尺寸（`components/Flag.tsx`）。简介则写在 `era.<时代>.<ID>.desc` 里，没写的国家回落到现代简介。

> **直播模式下**，德意志国的国旗会把万字换成「乐」，并在旗子下面标出灭亡日期（1945.5.8）。见 `src/game/flags.ts`。

**政体也随时代变**（`era.governments`）。民国是威权制度而不是共产主义——不写这一条的话，1938 年的中华民国会挂着现代中国的政体标签。同表还改了：德意志国、巴西（瓦加斯的新国家体制）为威权制度，大日本帝国、英属印度、伊朗帝国为君主制度；1994 年的俄罗斯是民主制度、朝鲜是共产主义。政体只影响显示（国家卡片、`status` 命令），目前不参与任何机制。

### 版图之内：谁在管事

1938 年的中华民国是一个国家、也是十几个政府。北伐没有解散军阀，只是让他们换了旗号：抗战是晋系在山西打、桂系在广西打、滇系在云南打、三马在西北打——打着重庆的旗，未必听重庆的话。新疆听盛世才的（背后是莫斯科），西藏自 1912 年起就自己管自己。

这些省在地图上**按省加一层暗色覆盖 + 虚线边界**，鼠标悬停国家会列出是谁在管，国家面板里也会写。**这只是着色**：国界不动，领土仍属中国，战争、和平、外交、割让全都照旧——`state.autonomous` 是一份独立的名单，不碰 `regionOwner`。

数据在 `eras.ts` 的 `WARLORD_CHINA` 表里（省名 + 派系，`faction.jin` → 晋系）。1945 年新疆已归国民政府（盛世才 1942 年与莫斯科翻脸、1944 年被重庆撤换），所以 `WARLORD_CHINA_1945` 少一省。本表由人编写，出处是各省在抗战期间的实际控制者——**这是全项目唯一一处手工整理的历史数据**，其余（国界、国旗、简介）都有外部来源。
